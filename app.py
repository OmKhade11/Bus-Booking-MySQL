...
def book_ticket(user_id, bus_id, travel_date, passenger_list): # Renamed train_id to bus_id
    ...
    try:
        ...
        # Changed table 'Trains' to 'Buses' and 'train_id' to 'bus_id'
        query_check_seats = """
        ...
        WHERE t.bus_id = %s
        GROUP BY t.bus_id, t.total_seats
        FOR UPDATE
        """
        
        cursor.execute(query_check_seats, (travel_date, bus_id))
        result = cursor.fetchone()
        
        # --- *** MODIFIED ERROR CHECKING *** ---
        if not result:
            print(f"Error: Bus ID {bus_id} not found.")
            conn.rollback()
            return False, f"Error: Bus ID {bus_id} does not exist."
        ...
        # Updated column train_id to bus_id
        query_insert_booking = """
        INSERT INTO Bookings (user_id, bus_id, travel_date, seat_count, status, total_fare, pnr_number)
        VALUES (%s, %s, %s, %s, 'CONFIRMED', %s, %s)
        """
        fare = 50.0 * seat_count_to_book  # Adjusted fare for bus
        # Changed PNR prefix to BUS
        pnr = f"BUS{user_id}{bus_id}{travel_date.replace('-', '')}" 
        
        cursor.execute(query_insert_booking, 
                       (user_id, bus_id, travel_date, seat_count_to_book, fare, pnr))
...
def handle_booking():
    try:
        ...
        bus_id = request.form['bus_id'] # Changed from train_id
        ...
        success, message = book_ticket(user_id, bus_id, travel_date_str, passengers)
